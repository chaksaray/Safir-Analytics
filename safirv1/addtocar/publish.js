...
Meteor.publish('productsWebzine', function(limit, val) {
    if (limit == -1 && val) {
        var getCat = contents.findOne({ "title": val });
        return products.find({ category: getCat.category }, { limit: 4 }); //return products.find({},{limit:limit});
    } else if (limit != -1) {
        return products.find({}, { limit: limit });
    } else {
        return products.find({});
